...
with tab_inv:
    ...
    with st.expander("📚 Understand the Math (Supply Chain Formulas)"):
        ...
        *   **Safety Stock (Buffer):** Extends inventory to protect against demand volatility and supplier delays.
            ...
            *Here, \(Z = {Z_score}\) corresponding to your selected {service_level*100}% service level.*
        ...
        *   **Economic Order Quantity (EOQ):** The classic formula that balances administrative ordering costs ($S$) against inventory warehouse storage costs ($H$).
            ...
            *Your holding cost is computed as {holding_cost_pct}% of the average unit price (${avg_unit_price:.2f}), yielding ${holding_cost_annual:.2f} per unit per year.*
        """.format(Z_score=z_score, service_level=service_level, holding_cost_pct=holding_cost_pct, avg_unit_price=avg_unit_price, holding_cost_annual=holding_cost_annual))
